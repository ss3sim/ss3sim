...
\fancyfoot[LO,RE]{\sffamily An introduction to \texttt{ss3sim}}
...
The CRAN version of \texttt{ss3sim} can be installed
in an \texttt{R} console with:
...
\noindent
The development version of \texttt{ss3sim} can be installed from GitHub
...
\noindent
We develop and test \texttt{ss3sim} around a specific version of SS3.
Currently, \texttt{ss3sim} is tested to work with SS3 Version 3.24O.
...
\url{https://github.com/ss3sim/ss3sim/tree/ss3-binaries/inst/ss3}.

\noindent
\texttt{ss3sim} requires the SS3 binary to be in your path.
This means that your operating system knows where the binary file is located.
See Appendix \ref{sec:path} for details.

Please direct any questions, suggestions, or bug reports to the \texttt{ss3sim}
issue tracker: \url{https://github.com/ss3sim/ss3sim/issues}, or to an author
as listed on the title page of this vignette.

If you use \texttt{ss3sim} in a publication, please cite the package as
indicated by running\\ \texttt{citation("ss3sim")} in the \texttt{R} console:
...
\texttt{ss3sim} relies on a set of plain text files to control the simulation (Figure~\ref{fig:files}).
...
\texttt{ss3sim} simulation. Folders are shown in blue, input files in orange,
...
and specify the simulation cases directly in \texttt{R} code (see section \ref{sec:ss3simbase}.}
\label{fig:files}
\end{figure}
...
An alternative approach to using \texttt{ss3sim} is to skip
...
The \texttt{ss3sim} package includes the capability for inducing time-varying
...
\texttt{ss3sim} is designed with a set of mandatory case IDs that must be
specified for each run: data (\texttt{D}), estimation (\texttt{E}), fishing
effort (\texttt{F}),and retrospective (\texttt{R}).
...
this within the \texttt{ss3sim} framework. Note that as currently developed,
\texttt{ss3sim} incorporates all new cases by using the \texttt{change\_tv}
...
created, you need to tell \texttt{ss3sim} to use them, since they are outside of
...
functions work outside of \texttt{ss3sim}, and then (2) running a single
...
\texttt{ss3sim} package developers for potential inclusion in future versions.

\section{Modifying the OM and EM models included with \texttt{ss3sim}}
\label{sec:modify}

\texttt{ss3sim} comes with three built-in SS3 operating and estimation model setups:
...
Further details on these models are available in \citet{johnson2013} and \citet{ono2013}.
...
\texttt{ss3sim} is not designed for testing arbitrary SS3 models, but rather properties of assessment models in general.
...
Here is a list of SS3 features that are not currently implemented in \texttt{ss3sim}:
\begin{itemize}
...
knowledge of SS3 and R, as well as modification of the \texttt{ss3sim}
...
places: indices of abundance, length compositions, and age compositions.
There are
...
\item[\texttt{change\_index}] This function samples from the biomass
  ...
  and survey index data. Since $q=1$ for both the OM and EM, the CPUE
...
Compared to sampling from the age and length compositions,
sampling from the abundance indices
is relatively straightforward. The OM \texttt{.dat} file contains the
...
the EM has unbiased estimates of $B_y$ and the true
$\sigma_y$ for all fleets.
...
Instead of copying the SS3 binary file (\texttt{ss3.exe}) to each folder
...
If needed, you can add to the path that \texttt{R} sees
