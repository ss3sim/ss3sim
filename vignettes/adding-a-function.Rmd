...
# Adding a new SS3 manipulation function to ss3sim. This function can be
  used by one or more cases. Cases can be mandatory (must be included in
  all scenarios, like `F` and `D`) or optional like `S` where if not
  specified then nothing occurs.
...
4. Go to `/R/run_ss3sim.r`.
  - `run_ss3sim` calls `ss3sim_base` so it needs to be updated to include your new case argument from step 3.
