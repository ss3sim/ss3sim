...
\texttt{ss3sim} comes with three built-in SS3 operating 
and estimation model setups: 
...
yellowtail flounder (\textit{Limanda ferruginea}, 
R. Methot, pers.\ comm.), 
and Pacific sardine \citep[\textit{Sardinops sagax caeruleus;}][]{hill2012}.
Further details on these models 
are available in \citet{johnson2013} and \citet{ono2013}.
...
While these model setups are generic 
and cover a wide range of life history types, 
they may not be suitable for all users.
Therefore, in this section, we outline strategies for modifying 
...
Instead of copying the SS3 binary file
(SS3 on Unix or SS3.exe on Windows) to each folder
...
This approach saves storage space since the SS3 binary is 2--3 mb and
having it located in each folder can be prohibitive 
in a large scale simulation testing study.
...
This stems from the high flexibility of SS3, 
allowing for more complex model setups
than those used while developing ss3sim. 
For instance, the \texttt{change\_index} function 
does not have the capability to
handle more than one season 
(it could, but currently it is not developed to). 
Given the myriad options available in SS3 it is extremely difficult to write
