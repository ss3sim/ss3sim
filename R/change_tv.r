...
change_tv <- function(change_tv_list,
  ...
  ## ADDED IN A SECTION TO BE ABLE TO CHANGE THE ENVIRONMENTAL VARIABLE IN EM TOO
  ## THIS NEED TO BE THOUGHT THROUGH AGAIN PROBABLY
  ## FOR NOW HOW IT WORKS IS THAT YOU GET THE OPTION TO CHANGE OM, EM OR BOTH OM AND EM
  ## TO CHANGE BOTH, NEED TO HAVE THE VALUES AS LISTS IN THE CASE FILE, AND ADD
  ## THE ARGUMENT model; c("om","em")
  ## THEN SEE ss3sim_base
  ## THERE ARE EXAMPLE CASE FILES IN THE new-cases folder (cases S)

  models_to_rm_from_main_list <- as.character(change_tv_list$models)
  change_tv_list$models <- NULL
  em_pars <- NULL

  if ( length(models_to_rm_from_main_list) > 1 & any(models_to_rm_from_main_list =="em"))
  {
    om_pars <-  change_tv_list[models_to_rm_from_main_list=="om"]
    em_pars <-  change_tv_list[models_to_rm_from_main_list=="em"]

  } else {
    models_to_rm_from_main_list <- "om"
    om_pars <-  change_tv_list
  }

   if(length(grep("om", getwd()))== 1) {
    change_tv_list <- om_pars
    ...
    env_line <- "-1 2 1 0 -1 99 -2 # env link specification i.e fixed to 1"
    sr_envline <- "-5 5 1 0 -1 99 -3 # SR_envlink"
    q_envline <- "-2 2 1 0 -1 99 -5 #"

    }
  if(length(grep("em", getwd()))== 1) {
    change_tv_list <- em_pars
    env_line <- "-1 2 1 0 -1 99 2 # env link specification i.e fixed to 1"
    sr_envline <- "-5 5 1 0 -1 99 3 # SR_envlink"
    q_envline <- "-2 2 1 0 -1 99 5 #"
  }
  ...
  if("mg" %in% tab$lab) {
    if(subset(tab, lab == "mg", select = "Freq") > 0 ) {
      ...
      ss3.ctl[(mg.ch + 1)] <- env_line                              # CHANGED THIS LINE ########################################
  ...
  if("sx" %in% tab$lab) {
    if(subset(tab, lab == "sx", select = "Freq") > 0 ) {
      ...
      ss3.ctl[(sx.ch + 1)] <- env_line                              # CHANGED THIS LINE ########################################
  ...
  if(length(temp.data) > 0) {
    ...
    ss3.ctl[sr.shortline.ch] <- sr_envline                              # CHANGED THIS LINE ########################################
    dat.varnum.counter <- dat.varnum.counter + 1
  ...
  for(i in seq_along(temp.data)) {
    ...
    paste.into.ctl <- c(paste.into.ctl,
                        paste(q_envline, names(temp.data)[i])) # CHANGED THIS LINE ########################################
   ...
   # for ( i in 1:length(env.name)){
    ...
    for(q in seq_along(change_tv_list)) {
      ...
      if(env.lab[q] == "sx") {
        ...
        num.sx_male <- grep("Sel_.Male", ss3.report) ############################################## INCLUDE THE MALE LINES FOR SEX SPEC
        num.sx <- sort(c(num.sx,num.sx_male)) ############################################## INCLUDE THE MALE LINES FOR SEX SPEC
        ...
        if (length(pos.sx) > 1) {
          ...
          if (!is.na(as.numeric(allchars[[1]][1]))) allchars <- allchars[[1]][-1] ###################### I THINK THIS WAS NEEDED TOO.....
