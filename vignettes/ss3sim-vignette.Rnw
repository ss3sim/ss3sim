...
\item If \texttt{sample\_mlacomp}, generate age-composition data
\item If \texttt{sample\_calcomp}, generate length- and age-composition data
\end{itemize}
...
If either of these is left set to \texttt{NULL} than the respective bin stucture
...
``truth'') can be added within the \pkg{ss3sim} framework in three
...
\begin{description}
\item[\texttt{sample\_index}] This function samples from the biomass
...
\item[\texttt{sample\_lcomp}] This function samples from the length
  ...
  information).
\item[\texttt{sample\_agecomp}] Same as length compositions except
...
annual biomass values, and the \texttt{sample\_index} function uses
