...
Here we introduce **ss3sim**, an \textsf{R} package that facilitates \R{A3:1}reproducible, flexible, and rapid end-to-end simulation testing with the widely-used SS3 framework. We begin by outlining the general structure of **ss3sim** and describing its functions, and then demonstrate the software with a simple example. We conclude by discussing how **ss3sim** complements other simulation testing software and by outlining some research questions that our freely accessible and general SS3 simulation-testing framework could address.
...
To demonstrate **ss3sim**, we will work through a simple example in which we examine the effect of (1) high vs. low precision of a fishery independent index of abundance and (2) \R{A20}fixing natural mortality ($M$) at an assumed value vs. estimating $M$. All files to run this example are included in the package data, and a more detailed description is available in the accompanying vignette (Text S1). **ss3sim** requires \textsf{R} version 3.0.0 or greater and SS3 (see Text S1 for more detailed instructions). In \textsf{R}, **ss3sim** can be installed and loaded with:
...
The high-level function `run_ss3sim` can run all simulation steps based on a specified scenario ID and a set of semicolon-delimited plain-text files that describe alternative cases \R{A21:2}(Figures 1 and 2). These case files contain argument values that are passed to the low-level **ss3sim** \textsf{R} functions (e.g. `change_index`, a function that controls how the fishery and survey indices are sampled; Table 1).
