...
Assuming you've set up \texttt{SS3} in your path and are familiar with how 
the package works (as explained below), you can run FISH 600 models as 
follows. Note that this example runs 1 iteration and 2 bias adjustment runs. 
...
Note that by default the ADMB output is suppressed so the models run faster.
You can turn this output back on by adding \texttt{ignore.stdout = FALSE} to
your call to \texttt{run\_fish600}:
...
If you're on Windows and want to suppress the ADMB output, you may also have to
add \texttt{show.output.on.console = FALSE} to \texttt{run\_fish600}.
...
You may want to run the iterations in chunks. In that case you'll want to tell 
\texttt{run\_ss3sim} that you've already run the bias correction by setting the 
\texttt{bias\_already\_run} argument to \texttt{TRUE} and the 
\texttt{bias\_adjust} argument to \texttt{FALSE}. When you run the bias 
correction runs you need to run at least one regular run. Therefore, you might 
...
machines would require copying the existing folder structure to the second
machine first.)

You can run multiple scenarios in sequence by expanding the vector of values 
passed to \texttt{scenarios}. For example,
...
Cole has written a function \texttt{expand\_scenarios}, which makes it easier to generate a list of scenarios. For example:
...
renamed \texttt{ss3.dat}, the control files have been renamed \texttt{om.ctl} 
