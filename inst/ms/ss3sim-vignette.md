...
To use the high-level wrapper function `run_ss3sim`, you will have unique case identifiers that combine to create unique scenarios. 
...
For example, `cod`, `sar`, or `fla` for cod, sardine, or flatfish. 
An example filename would therefore be `M1-sar.txt` or `lcomp2-fla.txt`. 
...
When we run the simulations, we'll pass our deterministic recruitment deviations to the function `run_ss3sim`. 
