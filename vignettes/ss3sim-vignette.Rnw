...
[Can we remove the .dat file from the documentation, is it ever used within the package?]
starter.ss
forecast.ss
\end{verbatim}
...
The high-level wrapper function \texttt{run\_ss3sim}, 
allows users to specify whether or not they would like to utilize 
the bias adjustment routine built into the package by setting
\texttt{bias\_adjust} to \texttt{TRUE} or \texttt{FALSE}.
The function runs each scenario a set number of times, 
as specified by \texttt{bias\_nsim}, 
to determine the appropriate maximum bias adjustment to use
during the maximum likelihood estimation mode. Using the bias adjustment procedure
helps assure that the estimated log-normally distributed recruitment deviations 
are mean unbiased leading to mean unbiased estimates of biomass \cite{methot2011}
...
phase \texttt{3} and initialize the estimation of \textit{M} at \texttt{0.21}.
...
The scenario names are written out in full for clarity, 
...
Process error will now be unique for each iteration but equal across scenarios
