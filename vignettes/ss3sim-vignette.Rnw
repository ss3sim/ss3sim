...
You can download SS3 at \url{http://nft.nefsc.noaa.gov/Stock_Synthesis_3.htm}.
...
You can specify any parameter defined in your SS3 model
...
effort (\texttt{F}), natural mortality (\texttt{M}), and restrospective
(\texttt{R}). In many instances a user may wish to include a new case ID beyond
these mandatory ones. This section is designed to explain how to accomplish
this within the \texttt{ss3sim} framework. Note that as currently developed,
\texttt{ss3sim} incorporates all new cases by using the \texttt{change_tv}
...
(time-varying or not) in the OM to more accurately mimic the behavior of a real
fishery. Using the help file for \texttt{change_tv}, creating a set of ``S''
(for selectivity) case argument files which contain the changes required. You
may only have a ``base case'' which applies to all scenarios in the simulation
...
the mandatory set. This is done through the \texttt{case_files} argument in the
\texttt{get_caseargs} function, which is passed through by the
\texttt{run_ss3sim}. In this case, we need to tell it to look for
case argument files beginning with ``S'' (see the help file for more info):
\begin{verbatim}
...
IDs. The data case ID is a special case because its case argument files do not
begin with \texttt{D}. This is because there are three functions associated with
the data case, independently 0modifying three different parts of the
\texttt{.dat} file. This provides an example of how you could incorporate
multiple changes to selectivity through different functions (which you would
...
then create R functions that modify selectivity in different ways, and pass them
the same way the \texttt{D} case is handled above. We recommend verifying the
...
\texttt{ss3sim} comes with three built-in SS3 operating and estimation model setups: a cod-like (slow-growing and long-lived), flatfish-like (fast-growing and intermediate-lived), and sardine-like (fast-growing and short-lived).
...
the myriad options available in SS3 it is extremely difficult to write
