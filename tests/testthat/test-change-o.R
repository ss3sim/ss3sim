...
test_that("change_o works when passing change_o_list", {
  ...
  report <- suppressWarnings(change_o(
    change_o_list = operat_params,
...
test_that("change_o works when passing par_name and par_int", {
  report <- suppressWarnings(change_o(
    par_name = "SR_LN(R0)", par_int = 16.8,
    ctl_file_in = "codOM.ctl",
    ctl_file_out = "codOM_modified.ctl"
  ))
...
test_that("change_o warns appropriately", {
  ...
  suppressWarnings(expect_error(change_o(
    change_o_list = operat_params,
    ctl_file_in = "codOM.ctl",
    ctl_file_out = "codOM_modified.ctl"
  )))
  ...
  suppressWarnings(expect_error(change_o(
    par_name = c("SR_LN(R0)", "x"), par_int = 16.8,
    ctl_file_in = "codOM.ctl",
    ctl_file_out = "codOM_modified.ctl"
  )))
  suppressWarnings(expect_error(change_o(
    par_name = "SR_LN(R0)", par_int = c(16.8, 1),
    ctl_file_in = "codOM.ctl",
    ctl_file_out = "codOM_modified.ctl"
  )))
  suppressWarnings(expect_error(change_o(
    par_name = "SR_LN(R0)", par_int = NULL,
    ctl_file_in = "codOM.ctl",
    ctl_file_out = "codOM_modified.ctl"
  )))
  suppressWarnings(expect_error(change_o(
    par_name = NULL, par_int = 16.8,
    ctl_file_in = "codOM.ctl",
    ctl_file_out = "codOM_modified.ctl"
  )))
})
