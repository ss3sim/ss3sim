# Tests are skipped on CRAN because the Stock Synthesis executable is not
# available.
...
test_that("A basic run_ss3sim scenario runs", {
  ...
  expect_equal(
    c(table(ssem$age_comp_fit_table[, c("Fleet", "Nsamp_in")])),
    c(19, 0, 0, 75)
  )
  ...
  expect_equal(bias_list$df$value[5], 0.98,
    tolerance = 0.01,
