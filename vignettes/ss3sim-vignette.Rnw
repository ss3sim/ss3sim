...
\section{Modifying the operating and estimation models}\label{sec:modify}
\texttt{ss3sim} comes with 3 built-in \texttt{SS3} operating and estimation
...
However we recognize that while these models are generic and cover a wide range
of life history types, they may not be suitable for all users. This section
details strategies for modifying the existing \texttt{SS3} models.

Before proceeding it is worth having a short discussion about the scope and
...
(e.g. \cite{johnson2013}) or between multiple EMs (e.g. \cite{ono2013}). As such
...
type models. That is to say that \texttt{ss3sim} is not designed for testing
...
\item Using the original cod model create the case argument files you desire for
  ...
  deterministic (section \ref{eq:deterministic}) run. After running, read in the
  data and do visual checks for proper functionality.
\item Find the original cod model. This can be found in the ``/models'' folder
  inside the package, located by
\begin{verbatim}
library(ss3sim)
d <- system.file("extdata", package = "ss3sim")
paste0(d, "/models/cod-om")
\end{verbatim}
...
\item Make a single change to either the \texttt{.dat} or \texttt{.ctl} files
  for the new model. This change cannot break either model -- run them manually
  if there is any question it might.
\item Rerun step 1 and again verify it is still working. If errors occur in the
  R function you will need to examine the function to determine why the error is
  occuring and fix by changing the R function and reloading it.
\item Repeat previous steps with all small changes until the models are
  satisfactory.
\item Turn off deterministic runs and run the simulation as with the built-in models.
\item If your new model works well with the package and is significantly
  ...
  managers for inclusion in future releases.
\end{itemize}
...
\section{Incorporating observation error}\label{sec:obs-error}
Observation error (i.e. uncertainty arising from sampling the
``truth'') can be added within the \texttt{ss3sim} framework in 3
...
\subsection{Sampling age and length compositions}\label{sec:comps}
...
\begin{align*}
  ...
  X&\sim N(0, \sigma_y)=\text{a random variable}
\end{align*}
...
150 years and 1000 iterations that are used if none are passed by the
...
across scenarios. For example if you have 2 scenarios and run 100
iterations of each, the same set of recruitment deviations are used
between these two scenarios. See section \ref{sec:reproduce} for more
information.
...
the functions in \texttt{ss3sim} were developed to work with these 3 models and
...
(section \ref{sec:om-modify}). It is likely that the \texttt{ss3sim} functions
will need to be modified in some way in the process, so the user should be
familiar with both \texttt{SS3} and R.

(fixme: should we make this bigger and its own section?)
Here is a list of \texttt{SS3} features that are not currently implemented:
\begin{itemize}
\item Seasons, sexes, hermaphrodism
...
the OM \texttt{.ctl} bins) so that they can easily be re-binned. Alternatively,
...
data is read in correctly and expected values of the population dynamics are
written to the \texttt{.dat} files (and sensical). We also advise manually
testing the \texttt{ss3sim} R functions that manipulate the OM (e.g.,
...
model in the sense that ADMB will run and produce MLE estimates of parameters;
it is intended to only be run for one iteration to generate the population
dynamics of the starting values.

\section{Setting up an estimation model}
\label{sec:em-setup}
Unlike the OM, the EM needs to be a valid ADMB model and run to achieve MLE
...
After completing the above steps run the model manually and verify that it loads
the data properly and the objective. If it works correctly, try running
...
\texttt{change\_e}). It is very possible that some of the functions will not
work perfectly with the new models. In this case, it may be necessary to modify
the \texttt{ss3sim} functions to be compatible with the new OM and EM.
