...
#'
#' Function was adapted from \code{\link{SS_varadjust}} as the function 
#' was not fully developed at the time it was needed for this package
#'
...
#' Default=TRUE.
#' @author Ian G. Taylor, Gwladys Lambert
#' @seealso \code{\link{SS_parlines}}, \code{\link{SS_changepars}}
#' @export
#'

SS_varadjust2 <- function(dir="C:/myfiles/mymodels/myrun/",
  ...
  ctl_lines[keyword_line] <- "1 #_Variance_adjustments_to_input_values"
  ...
  if (length(numeric_rows) <6){
    numeric_rows <- which(!is.na(as.numeric(ctl[,2])))
    ctl <- ctl[-1]
    maxcols <- maxcols-1
    }
  good_rows <- numeric_rows[1:6]
