...
TODO I'd like to pull out the ``general'' stuff from below 
(files needed, output structure, etc.) into a section here. 
And then keep the example more about the actual example.

\section{An example simulation with \texttt{ss3sim}}

As an example, we will run a 2x2 simulation design in which we test 
(1) the effect of increasing or decreasing research survey effort and 
...
(1) the folder with the plaintext case files, 
(2) the folder with the operating model (OM), 
(3) and the folder with the estimation model (EM).
...
To use the high-level wrapper function \texttt{run\_ss3sim}, 
we will have unique case identifiers that combine to create unique scenarios.
...
We can create template input files by running \texttt{create\_argfiles}. 
...
For example, \texttt{cod}, \texttt{sar}, or \texttt{fla} 
...
for the cod stock in the papers (cite our papers here). 
You can refer to these papers for details on how the models were set up.

[TODO need to reference (R. Methot, pers. comm.)] 

To investigate the effect of increasing or decreasing sampling effort, 
we will manipulate argument \texttt{sd\_obs\_surv} 
that gets passed to \texttt{change\_index()}. 
In case 1, we'll halve the standard deviation to 0.1
...
In case 0, we'll set the phase that \textit{M} gets estimated in 
to \texttt{-1} (any negative number will work).
In case 1, we'll set the phase that \textit{M} gets estimated in 
to 3 and initialize the estimation of \textit{M} at 0.21.
We can do this by including the line: \texttt{natM\_val; c(NA,-1)} 
in the file \texttt{E0-cod.txt} 
and the line: \texttt{natM\_val; c(0.21,3)} in the file \texttt{E1-cod.txt}.
...
We'll run some deterministic runs to check our model for bias
when process error is not included in the OM. 
To do this, we'll start by setting up a 
100 row (number of years) by 20 column (number of iterations) 
matrix of recruitment deviations, 
...
and copies over the operating and estimation models. 
...
the operating and estimation models have been checked 
to make sure they contain the minimal files (as listed above), 
the filenames have been made all lowercase, 
the data file has been renamed \texttt{ss3.dat}, 
the control files have been renamed \texttt{om.ctl} or \texttt{em.ctl}, 
and the starter and control files have been adjusted 
...
If you get a line like ``ss3 is not recognized \ldots{}'' 
...
\item
  In the ``Edit User Variable'' window 
  add to the \textbf{end} of the ``Variable value'' section
...
\item
  Run the assessment from a command window 
  using the command \texttt{ss3} to generate a \texttt{.par} file 
...
\item
  Modify the start and end years of the model 
