...
(e.g.\ M for natural mortality or S for selectivity).
...
type models. Thus, \texttt{ss3sim} is not designed for testing
arbitrary \texttt{SS3} models, but rather properties of assessment models in
general, and \texttt{ss3sim} is not ideal for quickly exploring properties of a
...
(2) then setting the \texttt{parallel = TRUE} in \texttt{run\_ss3sim}.
...
model in the sense that ADMB will run and produce MLE (maximum likelihood
estimation) estimates of parameters; it is intended to only be run for one
iteration to generate the population dynamics of the starting values.
