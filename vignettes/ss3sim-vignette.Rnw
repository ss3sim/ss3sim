...
in the OM. We recommend opening a command window inside your folder to help
...
can be checked by running \texttt{ss3\_24o\_safe -nohess} to make sure the
input files are read in properly and the model writes the files. Use the
\texttt{.ss\_new} files produced as a starting point so that the comments
match the instructions below.

You will see an error by ADMB that it cannot find the dat file because we
have specified a different file in the starter file.  You can safely ignore
this error.

\subsection{Starter file modifications}
\begin{enumerate}
\item Delete the starter.ss file and rename \texttt{starter.ss\_new} to
  \texttt{starter.ss}. Modify this new file.
\item
