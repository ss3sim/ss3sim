...
sample_fit_vbgf <- function(length.data, start.L1, start.L2, start.k,
  ...
  pars.mat <- matrix(nrow = 5, ncol = 3, rbind(
    c(start.L1, lo.L1, hi.L1),
  ...
  mod <- bbmle::mle2(get_vbgf_loglik,
    start = list(
      logL1 = transformed[1], logLinf = transformed[2],
  ...
  if (mod@details$convergence == 1 |
    ...
    out <- list(
      "L_at_Amin_Fem_GP_1" = 999, "L_at_Amax_Fem_GP_1" = 999,
    ...
    out <- list(
      "L_at_Amin_Fem_GP_1" = L1, "L_at_Amax_Fem_GP_1" = L.inf,
