#' Methods to include time varying growth features
#'
#' This function takes generic OM files for sardine, flatfish, and cod and
#' inserts time-varying growth only in the VB K. 
#'
...
#' between environmental data and K where a link parameter is
#' estimated: \code{M\' (y) = K + link * env(y)}
#' \item \code{"block"} is to allow M to be estimated separately in different
#' pre-specified time blocks
#' \item \code{"dev"} is to estimate a mean K and to estimate deviations about
#' that mean
#' }
#' @param ctl_file_in Input control file name (before K estimation method is
#' chosen)
...
#' @param n_blocks The number of time blocks within which K is estimated
...
#' path.
#' @author Kotaro Ono and Carey McGilliard, modified by Juan Valero for changing growth (only K)
#' @export 
#'
#' @details Although it appears that 3 options exist for how
#' time-varying K is modeled within SS and within this function, right
#' now only the \code{how_time_varying = "env"} (making K vary over time by
...
#' \dontrun{
#' # sardine model
#'  change_growth(how_time_varying="env",ctl_file_in = "control.ss_new",dat_file =
#'  "SardOM.dat",par_file = "SS3.par",n_blocks = 1,block_pattern =
#'  c(1990,2001),dev = rnorm(100,mean = 0,sd = 0.6))
#' 
#' # flatfish model
#'  change_growth(how_time_varying="env",
...
change_growth <- function(use=FALSE,dev, how_time_varying = "env", ctl_file_in =
  ...
  file.copy(from = ctl_file_in, to = "control_pre_change_growth.ss_new",
    overwrite = T, copy.mode = TRUE)
  file.copy(from = par_file, to = "pre_change_growth.par", overwrite = T,
    copy.mode = TRUE)
  file.copy(from = dat_file, to = "pre_change_growth.dat", overwrite = T, copy.mode = TRUE)
  ...
  if(how_time_varying == "env") {
    ...
    # find the line specifying growth params
    
    # Find the line # of the line with the character string "#
    # VonBert_K_Fem_GP_1" in SS_ctl
    ch3a <- grep("# VonBert_K_Fem_GP_1", SS_ctl) 
    ch3<-ch3a[1]
    
    # Extract every value that comes before "# VonBert_K_Fem_GP_1" on
    # line ch3 as characters in the character vector "val"
    ch4 <- regexpr("# VonBert_K_Fem_GP_1", SS_ctl[ch3])[1] 
    ...
    # find the line specifying growth params and specify
    # the environmental link
    val[8] <- -2
    val[10] <- year.beg
    val[11] <- year.end
    SS_ctl[ch3] <- paste(c(val, "# VonBert_K_Fem_GP_1"), collapse=" ")
    ...
    EnvLineNum =grep("VonBert_K_Fem_GP_1_ENV", SS_Report, fixed=TRUE) 
    LinkLine = SS_Report[EnvLineNum]
    EnvCol = regexpr("VonBert_K_Fem_GP_1_ENV", SS_Report[EnvLineNum], fixed=TRUE)[1]
  ...
  if(how_time_varying == "dev") {
    # add the time varying feature into the ctl file	 
    # find the line specifying growth params
    ch3 <- grep("# VonBert_K_Fem_GP_1", SS_ctl)
    ch4 <- regexpr("# VonBert_K_Fem_GP_1", SS_ctl[ch3])[1]
