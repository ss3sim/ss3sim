...
\texttt{ss3sim} is an R package that is meant to make it relatively quick and easy
...
For example:
\texttt{c(1, 2, 4)}, or \texttt{seq(1, 100)}, or \texttt{1:100}, or \texttt{matrix()}, or \texttt{NULL}.
...
for all subsequent iterations within that scenario.
We expect bias adjustment parameters applicable
...
for all the case files that we used in this example simulation. 
You can either download the source code 
or find this folder at the location contained 
in the \texttt{case\_folder} object defined in the previous chunk of R code.
...
You can specify any parameter, defined in your SS3 model, as time varying
through the \texttt{change\_tv} function.
The function works by adding an environmental deviate (env)
to the base parameter (\texttt{par}), 
creating a time varying parameter (\texttt{par'}):
$$par'_{y} = par + link * env_{y}$$.
...
log transform of the base parameter using the following formula:
...
stock recruit parameter that varies with time and you try to implent another,
...
\texttt{function\_type; change\_tv} in your case file. For example,
...
in which we describe \textit{M} as stationary at a constant value.
...
run_ss3sim(iterations = 1:20, scenarios =
  c("D1-E100-F0-R0-M0-cod",
    "D2-E100-F0-R0-M0-cod",
    "D1-E101-F0-R0-M0-cod",
    "D2-E101-F0-R0-M0-cod"),
...
x <- expand_scenarios(list(D = 1:2, E = 100:101, F = 0, M = 0, 
    R = 0), species = "cod")
...
ts_dat <- subset(ts_dat, replicate %in% 1:60)
...
scalar_dat <- subset(scalar_dat, replicate %in% 1:60)
...
you need to (1) register multiple cores or clusters and 
(2) then set the argument \texttt{parallel} in \texttt{run\_ss3sim} to TRUE.
For example, we could have run the previous example in parallel.
First, we'll register 4 cores:
...
\noindent
And then run our simulation on 4 cores simultaneously 
...
this example ran 1.9 times faster on 2 cores than on a single core,
and 3.2 times faster on 4 cores.

\section{Setting up an operating model}
\label{sec:om-setup}

\texttt{ss3sim} comes with 3 built-in \texttt{SS3} operating and estimation models:
...
North Sea cod (\textit{Gadus morhua}) (R. Methot, pers.\ comm.),
yellowtail flounder (\textit{Limanda ferruginea}) (R. Methot, pers.\ comm.),
and California sardine (\textit{Sardinops caeruleus}) \citep{hill2012}.
Further details on these models 
are available in \citep{johnson2013} and \citep{ono2013}.

Here, we outline the steps to take an existing \texttt{SS3} model 
and modify it to work with the \texttt{ss3sim} package. 
First, we cover setting up an operating model 
and then in section~\ref{sec:em-setup} we cover setting up an estimation model.
...
\item
  ...
  or a simulated one, as long as it conforms to the structure needed for SS3.
\item
...
\item
  If the model already contains the \texttt{ss3.par} file,
  skip this step and move to the next step.
  If the model doesn't contain the \texttt{ss3.par} file,
  run the assessment model from a command window
  using the command \texttt{ss3}
  to generate the \texttt{ss3.par} file
...
\item
  Run the model again with no estimation using the \texttt{ss3 - noest} command
\item
  Specify/change the specification of the model using the functions
  available in the \texttt{ss3sim} package
  e.g \texttt{change\_e}, \texttt{change\_sel}
\item
  Run the model again with no estimation using the \texttt{ss3 - noest} command
\end{enumerate}
...
\item Set the length bin method to 1 or 2 in the line labeled
   \texttt{\# length bin method}.
   Using a value of \texttt{1}, the bins refer to the population bins (=1cm).
   ...
   The length data bins MUST be wider than the population bin (=1),
...
\item
  Modify the \texttt{.ctl} file to specify the parameters phase,
  to include time-varying process (if wanted),
  and so that observed catches drive the catches.
\item
  Specify the forecast file if forecasts are desired.
  Use the \texttt{change\_e} function to perform forecasts.
\item
  Modify the \texttt{.dat} file
  so that it includes the user-specified process and observation errors.
  You can do this by using the functions in the \texttt{ss3sim} package.
  E.g \texttt{change\_index}, \texttt{change\_lcomp}
\end{enumerate}
...
\subsection{Data file modifcations}
The \texttt{data.ss\_new} files produced when executing the OM
contain the expected values of the OM population dynamics. The data
the EM model is fit to needs to be sampled with observation error
from these expected values in order to mimic the random sampling
process done with real fisheries data. The \texttt{ss3sim} package
provides three functions (detailed below) which carry out the random
sampling process and generate \texttt{.dat} files to be used in the
EM.

\begin{description}
\item[\texttt{change\_index}] This function samples from the
  biomass trends for the different fleets to simulate the collection
  of CPUE data. Since $q=1$ for both the OM and EM, the CPUE indices
  are actually absolute indices of biomass. The result of the random
  sampling is a log-normal observed CPUE that is centered at the
  expected value. The user can specify which fleets are sampled in
  which years and with what amount of noise. Note that the
  years/fleets specified to be sampled must be present in the OM
  file, so the user must setup the OM accordingly.
\item[\texttt{change\_lcomp}] This function samples from the
  length composition using a Dirichlet distribution. The Dirichlet
  ...
  \ref{sec:overdispersion}). The user can specify which fleets are
  sampled in which years and with what amount of noise. Further, the
  bin structure can be inputted as an argument and thus changed
  dynamically within a simulation (see \ref{sec:dybin} below for
  more information).
\item[\texttt{change\_agecomp}] Same as length compositions except
  acts on ages.
\end{description}

\subsubsection{Dynamic composition binning}
\label{sec:dybin}

The functions \texttt{change\_lcomp} and
\texttt{change\_agecomp} include optional arguments specifying the
bins to use. This option may be useful for users who wish to explore
the impact of different binning schemes of the same data. The user
may specify a vector of values determining how to bin the data. The
function works by determining which OM bins to collapse and summing
across these to form the new bins. Thus the specified bins need to
coincide (i.e. be a subset) with the OM bins. For instance, of the
OM bins are $b_1=(0,1,2,3,4,5)$ then this could be collapsed into
the bins $b_2=(0-1, 2-3, 4-5)$. Because of the way the function is
written it is paramount that the bins given by the user be
compatible with the OM bins. For instance, it is not possible
(within the framework provided here) to include bins outside of the
range of the OM bins. We have provided basic checks to the validity
of the user provided bin structure. However, \textbf{we stress that
  dynamic binning is a complicated process and the user is
  responsible for thoroughly testing their proposed binning
  structure}. See the help file for these functions for examples of
how to explore and test the functions. An alternative way of testing
bin structure would be to setup different OMs that have the bin
structures desired.
...
\section{Sampling age and length compositions}\label{sec:overdispersion}
When simulating assessment models we need a way to mimic the way
that the age/length compositions (comps) are sampled in reality.
Under perfect mixing of fish and truly random sampling
the distribution of samples would be multinomial (REF).
However, in practice neither of these are true
because the fish tend to aggregate by size and age,
and it is difficult to take random samples.
This causes the data to have more variance than expected,
i.e.\ be overdispersed (REF).
If a multinomial likelihood is assumed
and the true sample size is used then in effect
the model puts too much weight on the composition data,
at the cost of fitting other sources of data less well (REF).
Analysts thus need to ``tune'' their model
to find a more appropriate sample size
that more accurately reflects the information in the comp data.
...
Thus, there is a need to generate more realistic data that has overdispersion.
In this package we use the Dirichlet distribution
to generate such data for a specified level of overdispersion.

\subsection{Multinomial vs.\ Dirichlet distributions}
...
\subsection{Sampling with overdispersion}
...
The sampling functions provided in the package
allows you to specify either a multinomial or Dirichlet distribution,
as well as the level of overdispersion.
Note that the package does not allow
for tuning of the effective sample size,
and the argument \texttt{cpar} controls the level of overdispersion
in the sampling functions.
See the function documentation for
\texttt{change\_agecomp} and \texttt{change\_lcomp}
for more detail.

\section{Stochastic reproducibility}

In many cases, you may want to make
the observation and process errors reproducible.
For instance, you may want to reuse process errors
...
You can make the observation error reproducible
by setting the \texttt{seeds} argument in \texttt{ss3sim\_base}.
For example, you might want to do this when running \texttt{run\_ss3sim}.
The \texttt{seeds} argument takes a vector of seed values at least as long
as the number of iterations you are running in your simulation.

The process error (recruitment deviations)
can be set manually by the user
by passing a matrix of recruitment deviations
to the argument \texttt{user\_recdevs}.
The columns in the matrix represent iterations of the simulation,
and the rows represent years.
Ensure that you pass a matrix with at least enough columns and rows.
The \texttt{ss3sim} package contains a built in set of recruitment deviations
for up to 150 years and 1000 iterations.
If \texttt{user\_recdevs} are not specified,
then the package will use these built-in recruitment deviations.
See \texttt{?recdevs} for documentation on how they were generated.

Note that these built-in recruitment deviations are standard normal
and are multiplied by $\sigma_r$ (recruitment standard deviation)
as specified in the SS model, and bias adjusted within the package.
If you specify your own recruitment deviations,
the deviations are used exactly as specified (not multiplied by $\sigma_r$ as
specified in the SS model),
and it is up to you to bias correct them
by subtracting $\sigma_r^2 / 2$.
