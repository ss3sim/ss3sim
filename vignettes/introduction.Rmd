...
ss3sim is an R package to make it relatively quick and easy to run simulations with Stock Synthesis (SS). The package consists of a series of low-level functions that facilitate manipulating SS configuration files, running SS models, and combining the output and some wrapper functions to control simulation experiments, e.g., `run_ss3sim`. Much of this vignette is focused on how to effectively use `run_ss3sim`. If you use ss3sim in a publication, please cite the package as indicated by `citation("ss3sim")`. 

## Installation of ss3sim

ss3sim can be run on OS X, Windows, or Linux.
The CRAN version of ss3sim mirrors the master branch of the [github repository](https://github.com/ss3sim/ss3sim), which houses the code base. The development version contains the newest features that are not available in the CRAN version and can be installed from using devtools.
...
ss3sim was originally based on SS version 3.24O. The package was updated in 2019 to use a more recent version of SS and dedicated effort will be dedicated to keep the package up to date. The most recent version of SS that was tested for use with ss3sim is available on github (e.g., see [here](https://github.com/ss3sim/ss3sim/blob/master/inst/bin/Windows64/ss.exe?raw=true) for the Windows version) or can be obtained from the SS repository on [Vlab](https://vlab.ncep.noaa.gov/web/stock-synthesis/document-library/-/document_library/0LmuycloZeIt/view/5042555?_com_liferay_document_library_web_portlet_DLPortlet_INSTANCE_0LmuycloZeIt_redirect=https%3A%2F%2Fvlab.ncep.noaa.gov%3A443%2Fweb%2Fstock-synthesis%2Fdocument-library%3Fp_p_id%3Dcom_liferay_document_library_web_portlet_DLPortlet_INSTANCE_0LmuycloZeIt%26p_p_lifecycle%3D0%26p_p_state%3Dnormal%26p_p_mode%3Dview). ss3sim uses the safe version, which checks that vector, matrix, or array bounds are not exceeded. There is a slight overhead in this checking, but it may help identify issues. ss3sim requires that this binary, i.e., ss.exe, be in your path. This means that your operating system knows where the binary file is located. See [the vignette section on paths](#path) for details. 

## Help files

Please direct any questions, suggestions, or bug reports to the ss3sim
[issue tracker](https://github.com/ss3sim/ss3sim/issues). A brief, formal summary of the original package is available from @anderson2014a. More up-to-date help can be found using the help files available within the current package, which can be accessed using the standard `?functionname` notation, or by reading the vignettes (including this one), which can be accessed with `browseVignettes()`, e.g., 
...
See the folder `ss3sim/inst/extdata/eg-cases` inside the ss3sim source code for all the case files that are used in this example simulation. You can either download the [source code](https://github.com/ss3sim/ss3sim) or find the folder defined in the block of R code above this paragraph.
...
### Post-processing of results
  ...
  * calculate some useful values in new columns
...
```{r, relative-error-boxplots-det, fig.height=7, fig.width=5, fig.cap="Box plots of the relative error for deterministic runs. *M* is fixed at the true value from the OM (E100) or estimated (E101). The standard deviation on the survey index observation error is 0.001 (D100)."}
library("ggplot2")
...
```{r, relative-error-boxplots-sto, fig.height=7, fig.width=5, cache=TRUE, fig.cap="Box plots of relative error for stochastic runs. *M* is fixed at the true value from the OM (E0) or estimated (E1). The standard deviation on the survey index observation error is 0.4 (D1) or 0.1 (D0), representing an increase in survey sampling effort."}
...
If you want the different scenarios to have different process error you will need to make separate calls to `run_ss3sim` for each scenario and pass a different matrix of `user_recdevs` (see [the section on process noise](#pro-error)) or pass different iteration numbers.
...
This introduction vignette covers the basics of ss3sim. We have included additional vignettes if you wish to dive more deeply into [creating your own ss3sim functions](making-functions.html), [creating your own SS OM or EM model setups](making-models.html) for use with ss3sim, or [modifying existing OM and EM model setups](modifying-models.html).
