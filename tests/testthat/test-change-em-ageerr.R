...
test_that("change_em_ageerr works as expected", {
  fleets <- 1
  definition <- 2
  ...
  expect_equal(
    output$agecomp$ageerr[min(which((output$agecomp$fleet==fleets) == TRUE)):
                            max(which((output$agecomp$fleet==fleets) == TRUE))],
    rep(definition,((max(which((output$agecomp$fleet==fleets) == TRUE))+1)
                    -min(which((output$agecomp$fleet==fleets) == TRUE))))
  )
})
test_that("change_em_ageerr stops on error when expected", {
  expect_error(
    change_em_ageerr(
      dat_list = datalist,
      outfile = NULL,
