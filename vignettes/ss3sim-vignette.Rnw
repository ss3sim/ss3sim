...
We'll run some simulations to check our model for bias when process noise and
...
\noindent
We'll read in the \texttt{.csv} files:
...
\texttt{ss3sim} comes with three built-in \texttt{SS3} operating and estimation models: a cod-like (slow-growing and long-lived), flatfish-like (fast-growing and intermediate-lived), and sardine-like (fast-growing and short-lived).
These models are based on North Sea cod (\textit{Gadus morhua}) (R. Methot, pers.\ comm.), yellowtail flounder (\textit{Limanda ferruginea}) (R. Methot, pers.\ comm.), and California sardine (\textit{Sardinops caeruleus}) \citep{hill2012}.
...
In doing this, we removed many of the subtle features of the model.
While these models are generic and cover a wide range of life history types, they may not be suitable for all users.
...
The package was designed as a tool for examining structural differences in models.
...
of the models were not important and removed to produce a set of generic life-history-type models.
...
Thus \texttt{ss3sim} is not ideal for quickly exploring properties of a particular assessment model and other software packages should be explored if that is your goal.
...
it is difficult to take random samples \cite{pennington2002}.
This causes the data to have
more variance than expected, i.e.\ be overdispersed, and the effective sample
size is smaller \cite{hulson2012, maunder2011}.  If a
...
since it is unlikely to happen in practice \cite{pennington2002}.
...
sampled). However, with the Dirichlet distribution the effective sample size
