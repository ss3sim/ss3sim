...
You don't need to stick with the scenario ID scheme laid out
...
We'll start by copying over some case arguments built into the package.
We'll copy what was called
...
We apply the following rules if the arguments
to any sampling function are not \texttt{NULL}:
...
The binning structure for any age and length data is taken from the
values passed to the arguments
\texttt{len\_bin} and \texttt{age\_bin} in \texttt{ss3sim\_base}.
If either of these is left set to \texttt{NULL} than the respective bin structure
will be taken from the original OM.

\section{Generating observation error}
\label{sec:obs-error}
Observations, or data arising from sampling the ``truth'', can be added within
the \pkg{ss3sim} framework automatically and dynamically. Currently \pkg{ss3sim}
supports five types of data: indices of abundance, length and age compositions,
mean length (size) at age, empirical weight at age, and conditional age at
length.
These data are generated with
...
``sampling'' functions. Note that the years/fleets to be sampled must be present
in the OM output data file. \pkg{ss3sim} does this data creation dynamically but
in some cases it may need to be done manually (see section
\ref{sec:change-data})

In simulation work, the true underlying proportions
in the population are known and therefore
true multinomial sampling is possible.
...
When simulating these data  we are trying to mimic the way that
...
multinomial likelihood is assumed for overdispersed data,
and the true sample size is used
then in effect the model puts too much weight on the composition data,
at the cost of fitting other sources of data less well \citep{maunder2011}.
Analysts thus often ``tune'' their model to find a more appropriate
sample size that more accurately reflects the information in the composition
data.

\subsection{Indices of abundance}
The function [\texttt{sample\_index}] facilitates generating (relative) indices
of abundance.  It samples from the biomass trends for the different fleets to
simulate the collection of CPUE and survey index data. The user can specify
which fleets are sampled in which years and with what amount of noise. Different
fleets will ``see'' different biomass trends due to differences in selectivity,
so each index is typically associated with the same fleet between the OM and
EM. Since $q=1$ for the OM, the CPUE indices are actually absolute indices of
(spawning) biomass. Other assumptions about the distribution (overdispersion,
biased, etc.) about the truth are possible, but not currently supported.

In practice, sampling from the abundance indices is relatively
straightforward. The OM \texttt{.dat} file contains the annual biomass values
for each fleet, and the \texttt{sample\_index} function uses these true values
as the expected value. The function uses a bias-corrected log-normal distribution
with expected values given by the OM biomass and a user-provided standard
deviation term that controls the level of variance.

More specifically, let
\begin{align*}
  B_y&=\text{the true (OM) biomass in year $y$}\\
  \sigma_R&=\text{the standard deviation provided by the user}\\
  X\sim N(0, \sigma_R^2)&=\text{a normal random variable}
\end{align*}
Then the sampled value, $B_y^\text{obs}$ is
\begin{equation*}
  B_y^\text{obs}=B_y e^{X-\sigma_R^2/2}
\end{equation*}
which has expected value $\Eof{B_y^\text{obs}}=B_y$ due to the bias adjustment
term $\sigma_R^2/2$ (\texttt{SR_sigmaR} in SS3). This process generates
...
For index data, which are assumed log-normal by SS3, the weight of the data is
determined by the CV of each point. As the CV increases the data have less
weight in the joint likelihood. This is equivalent to decreasing the effective
sample size in other distributions. (TODO: \pkg{ss3sim} sets these values
automatically at the truth internally. Future versions may allow for more
complex options.)  The user-supplied $\sigma_R$ term is written to the
\texttt{.dat} file along with the sampled values, meaning that the EM has the
correct level of uncertainty. Thus, the EM has unbiased estimates of $B_y$ and
the true $\sigma_y$ for all fleets.

\subsection{Age and length compositions}
The functions \texttt{sample\_agecomp} and [\texttt{sample\_lcomp}] sample from
the true age and length compositions using a multinomial or Dirichlet
distribution. The user can further specify which fleets are sampled in which
years and with what amount of noise (sample size).
...
The composition sampling functions provided in the package allow you
...
\texttt{sample\_agecomp} and \texttt{sample\_lcomp} for more detail.

The Dirichlet distribution $\mathbf{d} \sim
\text{Dirichlet}\left (\boldsymbol{\alpha}, A\right )$ is used in place of the
multinomial to generate overdispersed samples. It is characterized by:
\begin{align*}
...
\texttt{sample\_agecomp} functions. In the case of the multinomial the effective
...
necessarily be integer-valued, and SS3 handles this without issue.
...
The functions \texttt{sample\_lcomp} and \texttt{sample\_agecomp}
