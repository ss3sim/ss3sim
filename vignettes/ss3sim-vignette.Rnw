...
Instead of copying the \texttt{SS3} binary file
(\texttt{SS3} on Unix or \texttt{SS3.exe} on Windows) to each folder
...
Given the myriad options available in \texttt{SS3} it is extremely difficult to write
