...
test_that("change_rec_devs() works with vector of recdevs", {
  ...
  out_ctl <- change_rec_devs(recdevs,
...
test_that("change_rec_devs works with named list", {
  ...
  read_yrs <- unlist(lapply(
    strsplit(trimws(read_yrs), "\\s+"),
    function(x) x[1]
  ))
...
test_that("change_rec_devs gives error when cannot find info on number and yrs for recdevs", {
  ...
  expect_error(
    ...
    paste0(
      "The number of recdevs and their associated years could",
      " not be determined"
    )
  )
})
...
test_that("change_rec_devs give error when advanced recdevs options are off", {
  ...
  expect_error(
    change_rec_devs(rnorm(101), "codOM_no_adv.ctl", NULL),
    paste0(
      "Currently ss3sim can only add recruitment deviations to ",
...
test_that("change_rec_devs looks for spot to put recdevs as expected", {
  ...
  expect_error(change_rec_devs(recdevs, "codOM_no_recdevs_cmt.ctl", NULL),
    paste0(
      "ss3sim could not find the line with comment ",
      "'#_read_recdevs' in the OM control file"
    ),
    fixed = TRUE
  )
  ...
  expect_error(
    change_rec_devs(recdevs, "codOM_missing_2_F_cmt.ctl", NULL),
