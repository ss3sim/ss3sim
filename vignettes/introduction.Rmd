...
`sample_*()` functions work together,
adding observation error to the expected values from the OM,
to create the input `.dat` file for the EM.
For example,
you might wish to sample conditional length-at-age data with
a certain bin structure for certain years and fleets.
To do that, expected values must be available from the OM for
length and age data for appropriate years and fleets at a sufficiently fine bin size.
One way around this would be to write an OM with
all possible data types,
for all possible years,
at an extremely fine bin structure.
Lots of data can substantially slow down how quickly the OM runs.
Therefore, functions within ss3sim modify the OM to include just
the data types and quantities that are needed
based on the arguments passed to `sample_*()`.
Internally,
ss3sim uses `calculate_data_units()` to generate all necessary expected values.
For example,
if arguments are passed to `sample_mlacomp()`,
then age and length-at-age expected values are created,
even if you do not want observed age-composition samples.
Similarly,
`sample_wtatage()` leads to expected values in the OM for
empirical weight-at-age, age-composition, and mean length-at-age data.
`sample_calcomp()` leads to expected values in the OM for
