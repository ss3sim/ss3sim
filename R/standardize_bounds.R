...
standardize_bounds <- function(percent_df, dir, em_ctl_file, om_ctl_file = "",
  ...
  if (!file.exists(file.path(dir, em_ctl_file))) {
    stop(paste(
      "The em_ctl_file,", em_ctl_file, "does not exist",
      "in the directory", dir
    ))
  }
  if (!"Label" %in% colnames(percent_df)) {
    stop(paste(
      "In percent_df, the first column is currently named",
  ...
  # Read in EM values
  em_pars <- r4ss::SS_parlines(
    ctlfile = file.path(dir, em_ctl_file),
  ...
  if (nchar(om_ctl_file) > 0) {
    ...
    om_pars <- r4ss::SS_parlines(
      ctlfile = file.path(dir, om_ctl_file),
      version = ss_version, verbose = verbose
    )
    ...
    indices <- sapply(percent_df$Label, function(x) {
      ...
      c(
        ifelse(is.null(findinom), NA, findinom),
    ...
    if (NROW(restr_percent_df) == 0) {
      stop(paste("None of the entered parameter labels (,",
        ...
        ") are found in both the EM and OM.",
        sep = ""
      ))
    }

    changeem <- cbind(
      em_pars$Label[indices[2, ]],
    ...
    if (NROW(changeinits) > 0) {
      ...
      print.verbose <- r4ss::SS_changepars(
        dir = dir, ctlfile = em_ctl_file,
      ...
      if (NROW(restr_percent_df) != 0) {
        ...
        if (any(whichunequal)) {
          ...
          r4ss::SS_changepars(
            dir = dir, ctlfile = em_ctl_file, newctlfile = em_ctl_file,
