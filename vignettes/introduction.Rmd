...
It seems imperative to first describe [Stock Synthesis][SS3]
...
hooilator parsed `data.ss_new` files into multiple files and
...
the directory structure that was created by hooilator.
Additionally, {ss3sim} added functionality to sample from the expected values
such that simulations were not forced to rely on bootstrapped values
...
See the [Stock Synthesis website][SS3] for a list of additional software.
...
Another option is to use process error that is not being modeled,
...
`sample_index()` facilitates generating relative indices of abundance
(catch-per-unit-effort data for fishery fleets).
It samples from the biomass trends available for the different fleets to
simulate the collection of catch-per-unit-effort and survey index data.
...
These values are estimated after an initial run and then written to the data file and Stock Synthesis is run again [@francis2011].
...
Conditional age-at-length data is an alternative to age-composition data,
when there are paired age and length observations of the same fish.
Conditional age-at-length data are inherently tied to the length compositions, e.g.,
...
Conditional age-at-length data are expected to be
more informative about growth than marginal age-composition data.
...
Process error is incorporated into the OM in the form of deviates in recruitment from the stock-recruitment relationship.
...
Note that for both built-in and user-specified recruitment deviations, {ss3sim} will reuse the same set of recruitment deviations for all iterations across scenarios.
...
$link$ is prespecified to a value of one.
