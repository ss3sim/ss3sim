...
test_that("sample_calcomp works", {
  ...
  len_bins <- unlist(lapply(
    strsplit(len_bins, split = "l"),
    function(x) x[2]
  ))
  ...
  expect_equivalent(
    exp_dat$agecomp[exp_dat$agecomp$Lbin_lo == -1, ],
    dat$agecomp
  )
  ...
  test_dif_bins <- cbind(test_dif_bins, data.frame(matrix(1,
    ncol = ncol(calcomp) - 9,
