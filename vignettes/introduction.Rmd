...
* a series of low-level functions that facilitate manipulating
  Stock Synthesis configuration files,
...
The developers of ss3sim strive to facilitate an inviting place where all individuals feel welcome.
Please see the
[Code of Conduct][code-of-conduct] that we abide by to ensure contributors are valued and
[Contributor Guidelines][contributor-guidelines] for how to contribute.
...
These `ss_new` files are produced by Stock Synthesis,
such that output that contains expected values is available with consistent formatting.
The developers of ss3sim built off of the ideas within hooliator and
added functions to sample from the expected values.
Sampling from expected values was atypical,
where most simulations typically relied on bootstrapped values that were provided in the Stock Synthesis output.
...
[ggplot2][ggplot2] was gaining traction at the time and shaped how we thought about things,
including our [mission statement](#overview) and [example output](beautiful-output).
...
If you are installing from GitHub, R will let you know if your current version is up to date.
...
The following code demonstrates how to access the ss3sim documentation within R.
You can also view the [vignettes online][ss3sim].
...
ss3sim relies the Stock Synthesis executable
being available to the operating system regardless of the directory.
Thus, `ss` or `ss.exe` must [be in your path](#path),
which is a list of directories that your operating system has access to.
Your operating system will search directories listed in your path when searching for programs.
Having [Stock Synthesis in your path](#path) allows users to
use just the name of the executable and not the full file path in a shell and
run the executable in a directory without copying it to the directory.
This functionality of not copying the executable works even when [running Stock Synthesis in parallel](#parallel).
This approach saves on storage space because the Stock Synthesis binary is about 6 MB.
It also facilitates manually running models during development and testing.
For example, you might be trying to create input files for a new simulation,
where the simulation is based on a life history that you are interested in.
Having Stock Synthesis in your path allows you to
open a shell prompt in the directory with your Stock Synthesis input files and
run the Stock Synthesis executable by typing `ss`.
...
ss3sim was originally based on Stock Synthesis 3.24O [@anderson2014a].
At first, ss3sim did not keep up to date with newer versions of Stock Synthesis.
In 2019, ss3sim was updated to use Stock Synthesis 3.30.
**Users can count on future versions of ss3sim being up to date with the newest version of Stock Synthesis.**
Stock Synthesis will end development with major-version three,
hence the name ss3sim.
But, minor versions will continue to be updated for quite some time.

#### Executables

Compiled Stock Synthesis executables are available for
multiple operating systems at the following locations:

* available [on github][ss3sim-bin],
* available [within GitHub actions in the Stock Synthesis GitHub repository][ss-githubactions], or
* are downloaded automatically when you use the GitHub version of ss3sim.

#### Putting Stock Synthesis in your path {#path}

ss3sim will first look to see if the executable is available in your R library using
`system.file("bin", package = "ss3sim")`.
The previous will only lead to a viable file path if
you are using the GitHub version of ss3sim because
the distribution of executables is not allowed on CRAN.
If the location is not returned from `system.file`,
R will look for the executable in your path.
This hierarchy is important for understanding which version of Stock Synthesis is being used.

If you are using the CRAN version of ss3sim,
reference the
[Getting Started with Stock Synthesis documentation][ss-gettingstarted]
for instructions on how to put Stock Synthesis in your path.

### Installing dependent R packages

Below we highlight a few packages that are essential for running ss3sim.

#### remotes

If you want to use the GitHub version of any R package,
you will need to install the [remotes package][remotes] first.
remotes can either be installed
directly using `install.packages("remotes")` or
will be available if you have [devtools][devtools].
...
Both methods will check to make sure that you have the most recent version of r4ss  in your R library available given which mirror you are using.
...
All output is available as csv files and
...
`"seq(1,10,by=2)"` and
`"c('NatM_p_1_Fem_GP_1', 'L_at_Amin_Fem_GP_1')"`.
...
The ss3sim package comes with a generic, built-in
...
```{r example-df}
# Use the boiler-plate data frame available in the package for 2 scenarios
...
`sample_*()` use the expected values from the OM output and
work together to create the input `.dat` file for the EM.
Note that the years/fleets to be sampled must be present in the OM data file.
ss3sim does this data creation dynamically [using `change_data()`](#change-data).
But, in some cases adding information to the `.dat` file may need to be done manually.
Currently ss3sim supports the sampling of the following types of data:

* indices of abundance with `sample_index()`,
...
In simulations, the true underlying dynamics of the population are known and therefore a variety of sampling techniques are possible.
Ideal sampling, in the statistical sense, can be done easily.
However, there is some question about the realism behind providing the model with this kind of data because
it is unlikely to happen in practice [@pennington2002].
Thus, there is a need to be able to generate more realistic data.
One way is to use flexible distributions,
...
The user-supplied recruitment deviations are used exactly as specified (i.e., not multiplied by $\sigma_r$ as specified in the Stock Synthesis model), and 
...
The package currently does not have built-in functions to turn on/off the estimation of time-varying parameters in the EM.
...
But, this can substantially slow down how quickly the OM runs in Stock Synthesis.
Therefore, we made ss3sim capable of dynamically modifying the OM to include just the data types and quantities that are needed based on arguments that are passed to `sample_*()`.
Internally, ss3sim uses `calculate_data_units()` to generate a superset of all necessary fleet and year combinations.
...
- If `sample_lcomp()`, generate length-composition data
...
For example,
if empirical weight-at-age data are to be sampled,
`change_data()` adds age-composition and mean length-at-age data to the OM data file.

Another feature of `change_data()` is that it can modify the binning structure of the data (not the population-level bins).
...
then the respective bins are taken from the OM.
