...
The high-level function `run_ss3sim` can run all simulation steps based on a specified scenario ID and a set of semicolon-delimited plain-text files that describe alternative cases \R{A21:2}(Figures 1 and 2). These files contain argument values that are passed to the low-level ss3sim \textsf{R} functions (e.g. `change_index`, a function that controls how the fishery and survey indices are sampled; Table 1).
