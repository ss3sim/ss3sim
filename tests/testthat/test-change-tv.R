...
test_that("change_tv() works for a MG param.", {
  ...
  expect_length(grep(
    "-2 2 1 0 99 0 -5 # TV_par_line for MG parameters",
    out$ctl_out
  ), 1)
})
...
test_that("change_tv() works for an MG param and an SR param", {
  ...
  expect_length(grep(
    "0.2 1 0.65 0.7 0.05 0 -3 202 0 0 0 0 0 0 # SR_BH_steep",
    out$ctl_out
  ), 1)
  expect_length(grep(
    "-2 2 1 0 99 0 -5 # TV_par_line for SR parameters",
    out$ctl_out
  ), 1)
})
...
test_that("change_tv() works for an SR param", {
  ...
  expect_length(grep("0.2 1 0.65 0.7 0.05 0 -3 201 0 0 0 0 0 0 # SR_BH_steep",
    out$ctl_out,
    fixed = FALSE
  ), 1)
  expect_length(grep(
...
test_that("change_tv() works for a q parm", {
  ...
  expect_length(grep("-20 20 0 0 99 0 -5 201 0 0 0 0 0 0 # LnQ_base_Survey(2)",
    out$ctl_out,
    fixed = TRUE
  ), 1)
  expect_length(grep(
    "-2 2 1 0 99 0 -5 # TV_par_line for Q parameters",
    out$ctl_out
  ), 1)
})
...
test_that("change_tv() works for a selectivity parm", {
  ...
  expect_length(grep(
    "-2 2 1 0 99 0 -5 # TV_par_line for selex parameters",
    out$ctl_out
  ), 1)
})
...
test_that("change_tv() works when there are 2 MG parms", {
  ...
  expect_length(grep(
    "0.001 1 0.2 0.25 0.8 0 4 202 0 0 0 0 0 0 # VonBert_K_Fem_GP_1",
    out$ctl_out
  ), 1)
  expect_length(grep(
    "-2 2 1 0 99 0 -5 # TV_par_line for MG parameters",
