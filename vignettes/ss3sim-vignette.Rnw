...
\item
  ...
  \texttt{\#do\_recdev} to \texttt{1}. Using the 
...
\item Specify \texttt{\# F\_Method} to \texttt{2},
   ...
   be specified in the \texttt{.dat} file. Next specify a line with six values, separated 
...
\item Specify the names for each fleet in an uncommented line after 
   \texttt{\#\_N\_areas}. Names must be separated by a \texttt{\%} with no spaces.
...
\item Specify the mean body weight across all selected sizes and ages
   ...
   to \texttt{0}. Subsequently, specify the degrees of freedom for the 
   Student's T distribution used to evaluated the mean body weight deviations to 
   \texttt{1} in the following line. The degrees of freedom must be specified
...
\section{Standardizing the model files}
