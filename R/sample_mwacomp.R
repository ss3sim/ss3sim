#' Sample mean weight (size-)-at-age data and write to file for use by the EM.
#'
...
sample_mwacomp <- function(datfile, outfile, ctlfile, fleets = 1, Nsamp,
    ...
    mlacomp <- mwacomp[mwacomp$AgeErr > 0, ]
    mwacomp <- mwacomp[mwacomp$AgeErr < 0, ]
    ...
    if(is.null(mwacomp)){
            stop("mean weight-at-age compositions do not exist")
        } else {
            if(nrow(mwacomp[mwacomp$AgeErr < 0, ]) < 1)
                stop("mean weight-at-age compositions do not exist")
        }
    ## End input checks
    ...
    mwacomp.new.list <- list() # temp storage for the new rows
    ...
    for(fl in 1:length(fleets)){
        fl.temp <- fleets[fl]
        mwacomp.fl <- mwacomp[mwacomp$Fleet == fleets[fl] & mwacomp$Yr %in% years[[fl]],]
        for(j in 1:NROW(mwacomp.fl)){
            yr.temp <- mwacomp.fl$Yr[j]
            mwacomp.new <- mwacomp.fl[j,]
            mla.means <- as.numeric(mwacomp.new[paste0("a",agebin_vector)])
            ## For each age, given year and fleet, get the expected weight
            ## and CV around that weight, then sample from it using
            ## lognormal (below)
            ...
            ## into the data frame.
            mwacomp.new.means <- do.call(c, lapply(lengths.list, mean))
            ## Sometimes you draw 0 fish from an age class,
            ## resulting in NaN for the mean mwacomp. For now,
            ## replace with filler values
### TODO: Fix the placeholder values for missing age bins
            mwacomp.new.means[is.nan(mwacomp.new.means)] <- -1
            ## mla data needs the sample sizes, so concatenate those on
            mwacomp.new[-(1:7)] <- c(mwacomp.new.means, age.samples)
            mwacomp.new.list[[k]] <- mwacomp.new
            k <- k+1
        }
    } # end sampling
    ## Combine new rows together into one data.frame
    mwacomp.new <- do.call(rbind, mwacomp.new.list)
    if(nrow(mlacomp) > 0) mwacomp.new <- rbind(mwacomp.new, mlacomp)
    datfile$MeanSize_at_Age_obs <- mwacomp.new
    datfile$N_MeanSize_at_Age_obs <- nrow(mwacomp.new)
    ## Write the modified file
    if(write_file) SS_writedat(datlist=datfile, outfile=outfile,
                                     overwrite=TRUE, verbose=TRUE)
    return(invisible(mwacomp.new))
}
