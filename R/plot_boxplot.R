...
#' \dontrun{
#' plot_boxplot(re, x = "E", y = "depletion_re", horiz = 'D',
#'   relative.error = TRUE)
#' }
#' rm(re)
...
#'   levels = c("om", "em"))
#' \dontrun{
#' plot_boxplot(ts_dat, x = "year", y = "SpawnBio",
...
plot_scalar_boxplot <- function(data, x, y,
  ...
  plot_boxplot(data = data, x = x, y = y,
...
plot_ts_boxplot <- function(data, y,
  ...
  fill = NA) {
  .Deprecated("plot_boxplot", package = "ss3sim")
  plot_boxplot(data = data, x = "year", y = y,
    horiz = horiz, horiz2 = horiz2,
    vert = vert, vert2 = vert2,
    relative.error = relative.error,
    axes.free = axes.free, print = print,
