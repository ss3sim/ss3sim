...
distribution of samples would be multinomial (REF). However, in practice
...
data to have more variance than expected, i.e.\ be overdispersed (REF). If
...
well (REF). Analysts thus need to ``tune'' their model to find a more
...
practice (REF). Thus, there is a need to generate more realistic data that
has overdispersion. In this package we use the Dirichlet
distribution to generate such data for a specified level of
overdispersion.

\subsection{Multinomial vs.\ Dirichlet Distributions}
The following calculations are shown for age compositions but apply equally to
...
\end{align*}
In the case of Stock Synthesis, actual proportions are input as
...
The sampling functions provided in the package allows you to
specify either a multinomial or Dirichlet distribution, as well as
the level of overdispersion. Note that the package does not allow
...
In many cases you may want to make the observation and
process errors reproducible. For instance, you may want to
reuse process errors so that differences between scenarios are not
confounded with process error. More broadly, you may want to
make a simulation reproducible on another machine.
